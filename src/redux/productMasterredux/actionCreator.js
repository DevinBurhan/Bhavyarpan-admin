...
export const uploadProductMasterAPI = (payload) => {
  return async (dispatch) => {
    try {
      const resp = await DataService.post(API.productMaster.upload, payload);
      if (resp.data.status) {
        message.success(resp.data.message);
        return true;
      } else {
  ...
  return async (dispatch) => {
    try {
      const resp = await DataService.put(
        API.productMaster.update + productMasterId,
        payload
      );
      if (resp.data && resp.data.status) {
  ...
  return async (dispatch) => {
    try {
      const resp = await DataService.delete(
        API.productMaster.delete + productMasterId
      );
      if (resp.data && resp.data.status) {
